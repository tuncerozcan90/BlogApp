...
async function populate() {
    ...
    if(count == 0) { 
        const users = await User.bulkCreate([
            {fullname: "özcan tuncer", email: "tuncerozcan90@gmail.com", password: await bcrypt.hash("1234", 10)},
            {fullname: "sami timur", email: "sami@gmail.com", password: await bcrypt.hash("1234", 10)},
            {fullname: "ramazan bayhan", email: "ramazan@gmail.com", password: await bcrypt.hash("1234", 10)},
            {fullname: "metin yalçın", email: "metin@gmail.com", password: await bcrypt.hash("1234", 10)},
            {fullname: "çınar turan", email: "info@cinarturan.com", password: await bcrypt.hash("1234", 10)},
        ]);
        ...
        await users[0].addRole(roles[0]); // admin => özcan tuncer
        await users[1].addRole(roles[1]); // moderator => sami timur
        await users[2].addRole(roles[1]); // moderator => ramazan bayhan
        await users[3].addRole(roles[2]); // guest => metin yalçın
        await users[4].addRole(roles[2]); // guest => çınar turan
